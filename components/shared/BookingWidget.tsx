...
export function BookingWidget({ calLink }: BookingWidgetProps) {
  ...
  // Normalize the link to a slug (strip protocol and host) and force embed mode
  ...
  return (
    <div className="flex flex-col gap-4 w-full h-full">
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 text-sm text-blue-800 flex items-start gap-3">
        ...
        <div>
          <p className="font-semibold">{t("booking.info_title")}</p>
          <p>
            {t("booking.info_prefix")} <a href="https://cal.com" target="_blank" rel="noopener noreferrer" className="underline">Cal.com</a>. {t("booking.info_suffix")}
          </p>
          <p className="mt-1 text-xs text-blue-600">
            {t("booking.info_page")}: <a href={baseUrl} target="_blank" rel="noopener noreferrer" className="underline">{baseUrl}</a>
          </p>
        </div>
      </div>
      ...
      <div className="w-full h-full min-h-[600px] border rounded-xl overflow-hidden bg-white relative shadow-sm">
        {isLoading && (
          <div className="absolute inset-0 flex items-center justify-center bg-neutral-50 z-10">
            ...
            <span className="ml-2 text-neutral-500">{t("booking.loading")}</span>
          </div>
        )}
        <iframe
          src={embedUrl}
